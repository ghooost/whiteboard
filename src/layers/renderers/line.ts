...
const Line = {
  postUpdateCurrent(layer: IDrawLayerLine, ctx: CanvasRenderingContext2D, startAt: number) {
    this.render(layer, ctx, startAt);
    return layer.points.length - 1;
  },

  render(layer: IDrawLayerLine, ctx: CanvasRenderingContext2D, startAt: number = 0) {
  ...
  change(layer: IDrawLayerLine, point: IDrawPoint) {
